  ...
  return (
    <>
      ...
      <div className='mt-10 flex gap-8 flex-col md:flex-row md:items-end'>
        <img className='w-48 rounded' src={albumData.image} alt="album chansons" />
        <div className='flex flex-col'>
            <p>Playlist</p>
            <h2 className='text-5xl font-bold mb-4 md:text-7xl'>{albumData.name}</h2>
            <h4>{albumData.desc}</h4>
            <p className='mt-1'>
                <img className='inline-block w-5' src={assets.spotify_logo} alt="logo spotify" />
                <b> Spotify </b>
                • 1 234 567 likes
                • <b>50 titres, </b>
                environ 1 h 45 min
      ...
      <div className='grid grid-cols-3 sm:grid-cols-4 mt-10 mb-4 pl-2 text-[#a7a7a7]'>
        ...
        <p className='hidden sm:block'>Lectures</p>
        <p>Album</p>
      ...
      <hr />
      {
        songsData.map((item,index)=>(
            <div onClick={()=>playWithId(item.id)} key={index} className='grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer'>
                ...
                <p className='text-[15px] hidden sm:block'>4 582 654</p>
